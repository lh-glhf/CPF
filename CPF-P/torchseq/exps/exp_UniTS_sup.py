from ..data_provider.data_factory import data_provider_pretrain as data_provider
from ..utils.tools import  cal_accuracy, adjustment
from ..utils.tools import adjust_learning_rate_pretrain as adjust_learning_rate
from ..utils.tools import NativeScalerWithGradNormCount as NativeScaler
from ..utils.metrics import metric
from ..loss.losses import mape_loss, mase_loss, smape_loss
...
from ..backbone.models import UniTS
...
class Exp_UniTS_sup(object):
    def __init__(self, args):
        super(Exp_UniTS_sup, self).__init__()
    ...
    def _build_model(self, ddp=True):
        ...
        if ddp:
            model = nn.DataParallel(model, device_ids=[self.device_id])
        return model
    ...
    def train(self, setting):
        ...
        path = path.replace("sup", "pretrain")
    ...
    def train_one_epoch(self, model_optim, data_loader_cycle, criterion_list, epoch, train_steps, scaler):
        ...
        for i, (sample_init, task_id) in enumerate(data_loader_cycle):
            ...
            if (i + 1) % 100 == 0:
                ...
                if is_main_process():
                    print(
                        "\titers: {0}, epoch: {1} | norm: {2:.2f} | loss: {3:.7f} | current_loss: {4} |current task: {5}".format(
                            i + 1, epoch + 1, norm_value, loss_sum_display / (i + 1), loss_display, task_name,
                            folder=self.path))
    ...
    def test(self, setting, load_pretrain=False, test_data_list=None, test_loader_list=None):
        ...
        if is_main_process():
            print("Avg score: LF-mse: {}, LF-mae: {}, CLS-acc {}, IMP-mse: {}, IMP-mae: {}, Ano-F: {}".format(
                avg_long_term_forecast_mse,
                avg_long_term_forecast_mae, avg_classification_acc, avg_imputation_mse, avg_imputation_mae,
                avg_anomaly_f_score), folder=self.path)
