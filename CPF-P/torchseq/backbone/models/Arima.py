    ...
    def fit(self):
        ...
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(data_loader):
            ...
            for var in range(enc_in):
                model = ARIMA(batch_x[:, :, var].squeeze().numpy(), order=(self.args.p, self.args.d, self.args.q),
            ...
            batch_y = batch_y.squeeze(axis=0).numpy()[-self.args.pred_len:, :]
            ...
            if i % 100 == 0:
                ...
                timer = time.time()
        print('mse:{}, mae:{}, rmse:{}, mape:{}, mspe:{}, smape:{}, wape:{}, msmape: {}'.format(np.average(mse_loss),
                                                                                                np.average(mae_loss),
                                                                                                np.average(rmse_loss),
                                                                                                np.average(mape_loss),
                                                                                                np.average(mspe_loss),
                                                                                                np.average(smape_loss),
                                                                                                np.average(wape_loss),
                                                                                                np.average(
                                                                                                    msmape_loss)))
        ii = 0
