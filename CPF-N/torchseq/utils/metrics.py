...
def metric_all(pred, true, concat=False):
    ...
    return ed, dtw, ddtw, peaks, entropy, avg, var
...
def calFileTest(rootPath):
    ...
    ed, dtw, ddtw, peaks, entropy, avg, var = metric_all(
        predWithoutRoll,
        trueWithoutRoll,
        concat=False)
    result_file = os.path.join(rootPath, "metrixPlus.txt")
    with open(result_file, "w") as f:
        f.write("Metrics for noRolling Test:\n")
        f.write(f"Euclidean Distance (ED): {ed}\n")
        f.write(f"Dynamic Time Warping (DTW): {dtw}\n")
        f.write(f"Derivative Dynamic Time Warping (DDTW): {ddtw}\n")
        f.write(f"Number of Peaks: {peaks}\n")
